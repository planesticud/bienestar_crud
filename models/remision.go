...
func init() {
	orm.RegisterModel(new(Remision))
}

// AddRemision insert a new Remision into database and returns
...
// GetRemisionById retrieves Remision by Id. Returns error if
// Id doesn't exist
...
// GetAllRemision retrieves all Remision matches certain condition. Returns empty list if
// no records exist
func GetAllRemision(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(Remision))
	// query k=v
	for k, v := range query {
...
// UpdateRemision updates Remision by Id and returns error if
...
// DeleteRemision deletes Remision by Id and returns error if
...
func DeleteRemision(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Remision{Id: id}); err == nil {
